...
/**
 * JUnit test class for testing the functionalities of {@link GoldCardCorner}.
 * This test class sets up a scenario with a {@link GoldDeck}, {@link PlayerField}, and various {@link ResourceCard}s,
 * then tests different methods of {@link GoldCardCorner} for correctness.
 */
public class GoldCardCornerTest {

    private GoldDeck goldDeck;
    private GoldCardCorner goldCardCorner;
    ...
    /**
     * Tests the {@link GoldCardCorner#getRequirements(boolean)} method of {@link GoldCardCorner}.
    ...
    public void getRequirementsTest() throws PlayerExceptions {
        while (!goldDeck.isEmpty()){
            ...
            if (goldCard instanceof GoldCardCorner){
                goldCardCorner = (GoldCardCorner) goldCard;
                assertTrue(goldCardCorner.getRequirements(false).isEmpty());
                Map<Symbol, Integer> requirements = goldCardCorner.getRequirements(true);
    ...
    /**
     * Tests the {@link GoldCardCorner#checkRequirements(PlayerField, boolean)} method of {@link GoldCardCorner}.
    ...
    public void checkRequirementsTest() throws PlayerExceptions {
        while (!goldDeck.isEmpty()) {
            ...
            if (goldCard instanceof GoldCardCorner) {
                goldCardCorner = (GoldCardCorner) goldCard;
                assertTrue(goldCardCorner.checkRequirements(field, false));
                Map<Symbol, Integer> requirements = goldCardCorner.getRequirements(true);
                ...
                assertEquals(are_requirements_met, goldCardCorner.checkRequirements(field, true));
            }
        }
    }

    /**
     * Tests the {@link GoldCardCorner#getCorner(CornerPosition, boolean)} method of {@link GoldCardCorner}.
        ...
        while (!goldDeck.isEmpty()) {
            ...
            if (goldCard instanceof GoldCardCorner) {
                ...
                for (CornerPosition pos : CornerPosition.values()) {
                    corner = goldCardCorner.getCorner(pos, false);
                    assertTrue(corner.IsPresent());
                    assertEquals(corner.getSymbol(), Symbol.EMPTY);
                    corner = goldCardCorner.getCorner(pos, true);
                    if (!corner.IsPresent()) {
                        try {
                            corner.getSymbol();
                            fail();
    ...
    /**
     * Tests the {@link GoldCardCorner#getSymbolsOnSide(boolean)} method of {@link GoldCardCorner}.
    ...
    public void getSymbolsOnSideTest() throws PlayerExceptions {
        ...
        while (!goldDeck.isEmpty()) {
            ...
            if (goldCard instanceof GoldCardCorner) {
                goldCardCorner = (GoldCardCorner) goldCard;
                ArrayList<Symbol> symbols = goldCardCorner.getSymbolsOnSide(false);
                assertEquals(1, symbols.size());
                assertEquals(symbols.getFirst(), goldCardCorner.getKingdom());
                symbols = goldCardCorner.getSymbolsOnSide(true);
    ...
    /**
     * Tests the {@link GoldCardCorner#getKingdom()} method of {@link GoldCardCorner}.
    ...
    public void getKingdomTest() throws PlayerExceptions {
        while (!goldDeck.isEmpty()) {
            ...
            if (goldCard instanceof GoldCardCorner) {
                ...
                assertEquals(goldCardCorner.getKingdom(), topSymbol);
            }
        }
    }

    /**
     * Tests the {@link GoldCardCorner#getScorePerCorner()} method of {@link GoldCardCorner}.
     * Verifies that the score per corner is calculated correctly.
     * @throws PlayerExceptions if there's an issue with player-related exceptions.
     */
    @Test
    public void getScorePerCornerTest() throws PlayerExceptions {
        while (!goldDeck.isEmpty()) {
            GoldCard goldCard = (GoldCard) goldDeck.draw();
            if (goldCard instanceof GoldCardCorner) {
                boolean invalidScore = false;
                goldCardCorner = (GoldCardCorner) goldCard;
                int score = goldCardCorner.getScorePerCorner();
                if (score != 2){
                    invalidScore = true;
                }
    ...
    /**
     * Tests the {@link GoldCardCorner#calcScore(PlayerField, boolean, Position)} method of {@link GoldCardCorner}.
    ...
    public void calcScoreTest() throws PlayerExceptions {
        while (!goldDeck.isEmpty()) {
            ...
            if (goldCard instanceof GoldCardCorner) {
                goldCardCorner = (GoldCardCorner) goldCard;
                assertEquals(0, goldCardCorner.calcScore(field, false, new Position(2, 0)));
                assertEquals(0, goldCardCorner.calcScore(field, false, new Position(-1, 1)));
                assertEquals(goldCardCorner.getScorePerCorner()*4, goldCardCorner.calcScore(field, true, new Position(2, 0)));
                assertEquals(goldCardCorner.getScorePerCorner()*2, goldCardCorner.calcScore(field, true, new Position(-1, 1)));
                assertEquals(goldCardCorner.getScorePerCorner()*3, goldCardCorner.calcScore(field, true, new Position(4, -2)));
            }
        }
    }
    ...
    public void toStringTest() throws InvalidSymbolException {
        ...
        GoldCardCorner card = new GoldCardCorner(Symbol.ANIMAL, frontCorners2, requirements, 2, "000");
